...
describe('Daypass purchases', () => {
  ...
  context('Parameters', () => {
    it('should require required parameters', (done) => {
      agent.post('/api/purchase/daypass')
        .send({ amount: 0, email: '@', source: { id: 'x' } })
        .expect((res) => {
          const exp = { status: 400, message: 'Required parameters: amount, email, passes, source' };
    ...
    it('should require at least one pass with a day', (done) => {
      agent.post('/api/purchase/daypass')
        .send({ amount: 1, email: '@', passes: [{ legal_name: 'y', membership: 'Adult' }], source: { id: 'x' } })
        .expect((res) => {
          const exp = { status: 400, message: 'All passes must include at least one day' };
          if (res.status !== exp.status) throw new Error(`Bad status: got ${res.status}, expected ${exp.status}`);
          if (res.body.message !== exp.message) throw new Error(`Bad reply: ${JSON.stringify(res.body)}`);
        })
        .end(done);
    ...
    it('should require a correct amount', (done) => {
      agent.post('/api/purchase/daypass')
        .send({ amount: 1, email: '@', source: { id: 'x' }, passes: [{ membership: 'Adult', email: '@', legal_name: 'x', day1: true }] })
        .expect((res) => {
          const exp = { status: 400, message: `Amount mismatch: in request 1, calculated 2500` };
  ...
  context('Prices', function() {
    ...
    it('should get prices', (done) => {
      agent.get('/api/purchase/daypass-prices')
        .expect(200)
        .expect(({ body }) => {
          if (
            !body || !body.Adult || !body.Adult.day1
          ) throw new Error(
